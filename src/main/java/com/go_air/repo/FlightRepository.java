...
public interface FlightRepository extends JpaRepository<Flights, Long> {
	...
	@Query("SELECT f FROM Flights f WHERE " +
		       "(:airline IS NULL OR LOWER(CAST(f.airline AS text)) = LOWER(:airline)) AND " +
		       "(:sourceAirport IS NULL OR LOWER(CAST(f.sourceAirport AS text)) = LOWER(:sourceAirport)) AND " +
		       "(:destinationAirport IS NULL OR LOWER(CAST(f.destinationAirport AS text)) = LOWER(:destinationAirport)) AND " +
